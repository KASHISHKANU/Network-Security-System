...
async def predict_route(request: Request, file: UploadFile = File(...)):
    try:
        ...
        df["threat_label"] = df["predicted_column"].apply(
            lambda x: "MALICIOUS" if x == 1 else "SAFE"
        )

        total = len(df)
        malicious = int((df["predicted_column"] == 1).sum())
        safe = total - malicious
        threat_percent = round((malicious / total) * 100, 2)
